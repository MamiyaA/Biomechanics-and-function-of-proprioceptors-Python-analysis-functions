...
class AxonRecording:
  ...
  def filter_ScanImageFile(self):
    ...
    depth_avg_image_GCaMP=np.zeros((GCaMPSignal.shape[0]//n_of_z,GCaMPSignal.shape[1],GCaMPSignal.shape[2]))
    ...
    depth_avg_image_GCaMP=depth_avg_image_GCaMP/n_of_z
    ...
    depth_avg_image_tdTomato=np.zeros((tdTomatoSignal.shape[0]//n_of_z,tdTomatoSignal.shape[1],tdTomatoSignal.shape[2]))
    ...
    #save the depth_avg_image
    depth_avg_image_tdTomato=depth_avg_image_tdTomato/n_of_z
    image_file_name=file_name.split('.')
  ...
  def motion_correction(self):
    ...
    #filtered_images is np array with [frames, rows, columns]
  ...
  def get_piezo_response_map(self):
    """
    a method to generate the DF/F and DR/R response map for the piezo stimulation:
    load the filtered and registered data for both tdTomato and GCaMP and calculate
    the DF/F and DR/R map during the first piezo stimulus.
    *tdTomato_file: a pickle file that contains the filtered and registered tdTomato images.
    Each object should have a path to this file
    *GCaMP_file: same for the GCaMP.
    ...
    *min_range and max_range defines the min and max for the DF/F and DR/R images.
    ...
    #calcuate the baseline and the response images.
    average_tdTomato=np.average(tdTomato_registered[first_piezo_start:first_piezo_start+response_range,:,:],axis=0)
    average_gcamp=np.average(gcamp_registered[first_piezo_start:first_piezo_start+response_range,:,:], axis=0)
    base_tdTomato=np.average(tdTomato_registered[0:first_piezo_start,:,:],axis=0)
    base_gcamp=np.average(gcamp_registered[0:first_piezo_start,:,:],axis=0)

    ratio_response=np.divide(average_gcamp,average_tdTomato)
    ratio_baseline=np.divide(base_gcamp,base_tdTomato)
    ...
    #DR/R calculated only for pixels whose base_gcamp value is above threshold
    DR_R_map=np.divide((ratio_response-ratio_baseline),ratio_baseline,where=(base_gcamp>=gcamp_threshold))
